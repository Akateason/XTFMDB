...
- (NSArray *)methodListInfo {
    ...
    for (int i = 0; i < count; i++) {
        ...
        for (int index = 0; index < argumentsCount; index++) {
            ...
            //            NSString *argString = [NSString stringWithCString:arg
            //            encoding:NSUTF8StringEncoding];
...
+ (NSDictionary *)dictionaryWithProperty:(objc_property_t)property {
   ...
   G<name>     | The property defines a custom getter selector name. The name
   follows the G (for example, GcustomGetter,).
   S<name>     | The property defines a custom setter selector name. The name
   follows the S (for example, ScustomSetter:,).
   ...
   */
    ...
    if ([attributeDictionary objectForKey:@"T"]) {
     ...
     i               An int
     s               A short
     l               A long l is treated as a 32-bit quantity on 64-bit
     programs.
     q               A long long
     C               An unsigned char
     I               An unsigned int
     ...
     *               A character string (char *)
     ...
     #               A class object (Class) :               A method selector
     (SEL)         [array type]    An array
