...
+ (NSString *)xt_getStrWithTick:(long long)tick {
    return [self xt_getStrWithTick:tick format:kTIME_STR_FORMAT_1];
}
...
- (NSString *)xt_timeInfo {
   ...
   // 这里按月最大值来计算
   // 获取发布日期中，该月总共有多少天
   int totalDays = [NSDate daysInMonth:(int)[date getMonth] year:(int)[date
   getYear]];
   // 当前天数 +
   （发布日期月中的总天数-发布日期月中发布日，即等于距离今天的天数）
   ...
   return (retDay < 4) ? [NSString stringWithFormat:@"%d天前", (abs)(retDay)] :
   [self getMMDDWithDate:date] ;
...
+ (NSDate *)xt_getDateWithStr:(NSString *)dateStr {
    return [self xt_getDateWithStr:dateStr format:kTIME_STR_FORMAT_1];
}
